...
target_features = []    # F'
rewards = []           # R
done_flags = []        # Terminal state indicators
...
for episode in range(20000): 
    ...
    for step_num in range(maxSteps):
        ...
        target_features.append(next_state_features)
...
target_features = torch.tensor(np.array(target_features), dtype=torch.float32)
rewards = torch.tensor(np.array(rewards), dtype=torch.float32)
done_flags = torch.tensor(np.array(done_flags), dtype=torch.bool)

print("\nFeature transition dataset ready:")
print(f"F + A shape: {input_features.shape}")
print(f"F' shape: {target_features.shape}")
print(f"Rewards shape: {rewards.shape}")
print(f"Done flags shape: {done_flags.shape}")
print(f"Total transitions collected: {len(input_features)}")
print(f"Unique (state, action) pairs visited: {len(visited)} / {num_states * num_actions}")
print(f"Coverage: {len(visited) / (num_states * num_actions):.1%}")

# Print reward statistics
# print(f"\nReward Statistics:")
# print(f"Positive rewards (goal): {(rewards > 0).sum().item()}")
# print(f"Negative rewards (holes): {(rewards < 0).sum().item()}")
# print(f"Zero rewards (normal): {(rewards == 0).sum().item()}")
# print(f"Terminal transitions: {done_flags.sum().item()}")

# MODIFIED: Save the complete dataset
dataset = {
...
torch.save(dataset, 'transition_features_with_rewards.pt')
print("Complete dataset saved to 'transition_features_with_rewards.pt'")

# Also save individual components for backward compatibility
torch.save((input_features, target_features), 'transition_features.pt')
print("Basic dataset saved to 'transition_features.pt'")
...
def load_transition_dataset(filename='transition_features_with_rewards.pt'):
    ...
    Returns:
        dict with keys: 'input_features', 'target_features', 'rewards', 'done_flags'
    """
...
print(f"Sample done flag: {loaded_dataset['done_flags'][0].item()}")