# V-Prime Training Using Stored Transitions (No Environment Steps Needed)
...
v_prime = VNetworkFeature(feature_dim=32)
gamma = 0.9


###----------------------------------------------------------------------------------------------------------------
def train_vprime_with_improved_mdn(dataset, current_reward, v_prime, mdn_improved, v_network, num_epochs=4000, gamma=0.2):
    """
    Train V-prime using the improved MDN - process each unique state individually
    """
    # input_features = dataset['input_features']
    
    # # Extract unique state features (first 32 dims)
    ...
    print(f"Training V-prime with improved MDN for {num_epochs} epochs...")
    print(f"Found {len(unique_states)} unique states")
    
    optimizer = optim.Adam(v_prime.parameters(), lr=0.001, weight_decay=1e-5)  # Reduced LR
    loss_fn = nn.MSELoss()
    ...
    for epoch in range(num_epochs):
        ...
        for state_idx, features in enumerate(unique_states):
            #current_state, _ = find_most_similar_state_to_letters(features, v_network) # for Debugging
            with torch.no_grad():
                mdn_improved.eval()
                v_prime.eval()
                ...
                for action in actions:
                    ...
                    # Get MDN prediction
                    pi, sigma, mu, reward_logits, pred_done = mdn_improved(state_action_input)
                    ...
                    # Get most likely next state (best mixture)
                    best_mixture_idx = torch.argmax(pi, dim=1)
                    next_features = mu[0, best_mixture_idx[0]]

                    next_value = target_v_prime(next_features.unsqueeze(0)).squeeze().item()
                    #next_value = np.clip(next_value, -10.0, 10.0)  # Force reasonable range

                    reward = current_reward['rewards'][state_idx].item()  # Get reward from dataset
                    if reward < 0: 
                        reward = -10
                    
                    q_value = reward + 0.9 * next_value 
                    target_value = max(target_value, q_value)
                    #action_values.append(q_value)
                
                # V(s) = max_a Q(s,a) - Bellman target
                #target_value = max(action_values)
                #target_value = np.clip(target_value, -10.0, 10.0)  # Clamp target
            
            # Train on this single state immediately
            v_prime.train()
            optimizer.zero_grad()
        ...
        # Print progress with early stopping check
        if epoch % 100 == 0:
...
def main():
    ...
    v_prime_final = train_vprime_with_improved_mdn(
        dataset, 
        current_reward,
        v_prime_improved, 
